...
import decisionMakingtest
import monitorExchangeRatetest
        ...
        if (tokyoTime.hour >= 10):
            if (tokyoTime.hour <= 10):
                if(tokyoTime.minute <=10):
                    lib.decisionMakingtest.mainexecuting("h10", japanTime, todays_date)
            elif(tokyoTime.hour <=11):
                if(tokyoTime.minute <=10):
                    lib.decisionMakingtest.mainexecuting("h11", japanTime, todays_date)
                    
            elif(tokyoTime.hour <=12):
                if(tokyoTime.minute <=10):
                    lib.decisionMakingtest.mainexecuting("h12", japanTime, todays_date)
...
def letsGetStarted():
    repeat = 100
    # jobfirst = Thread(target=monitorExchangeRatetest.job1, args=(repeat,))
    # jobsecond = Thread(target=decisionMakingtest.job3, args=(repeat,))
    # # # jobthird = Thread(target=lib.stopOrLimitOrder.job3)
    # jobfirst.start()
    # jobsecond.start()
    # # jobthird.start()
    # monitorExchangeRatetest.job1(repeat)
    decisionMakingtest.job3(repeat)
